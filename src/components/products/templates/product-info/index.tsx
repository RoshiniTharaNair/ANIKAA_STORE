...
{fullAncestorCategories.length > 0 && (
  <div className="ancestor-categories">
    {/* Add the "All Outfits >" text before the category breadcrumbs */}
    <span className="text-medium text-[#56242e]" style={{ fontWeight: "bold", fontSize: "15px", marginRight: "10px" }}>
      All Outfits &gt;
    </span>
    {fullAncestorCategories.map((category, index) => {
      ...
      return (
        <React.Fragment key={category.id}>
          {index > 0 && " > "}
          <Link href={`/${category.handle}`} className="text-medium text-[#56242e]">
            <span className={index === fullAncestorCategories.length - 1 ? "bold-text" : ""} style={{ fontSize: "15px", fontWeight:"bold" }}>
