...
import "react-toastify/dist/ReactToastify.css"; // Make sure this is imported correctly
...
const validatePassword = (password: string) => {
  ...
  return errors.length === 0 ? true : `Password ${errors.join(', ')}.`;
};
...
const Register = () => {
  ...
  const { data: session } = useSession(); // Hook to access session details
  ...
  const handleGoogleSignIn = async () => {
    try {
      ...
      // Once the user is signed in, extract Google account details and token
      if (session?.user) {
        const googleUser = session.user;
        // const token = session?.accessToken; // Assuming you're getting the token from the session

        // console.log("session ", session);

        // Set the extracted credentials for form submission
        const googleCredentials: RegisterCredentials = {
          first_name: googleUser?.name?.split(" ")[0] || "",
          last_name: googleUser?.name?.split(" ")[1] || "",
          email: googleUser?.email || "",
          password: googleUser?.email || "",  // Handle password accordingly (can generate or prompt user)
          phone: "", // You can fetch or set this as needed
        };

        // console.log("Google Account Details:", googleCredentials);

        const response = await medusaClient.auth.exists(googleCredentials.email);

        if (response.exists) {
          // console.log("Customer already exists. Please log in.")
          toast.error("Customer already exists. Please log in.");
          setCurrentView(LOGIN_VIEW.SIGN_IN);
          router.push("/account");
          return;
        }
        // Call createCustomer with the Google account details and token
        await createCustomer(googleCredentials);
      }
    } catch (error) {
...
const onSubmit = handleSubmit(async (credentials) => {
  ...
  // Validation checks: Ensure either email or phone is provided
  if (!credentials.email && !credentials.phone) {
    toast.error("Please provide either an email or phone number.");  // Display error if both are missing
    return;
  }

  if (credentials.phone && !phonePattern.test(credentials.phone)) {
    toast.error("Invalid phone number");  // Display error for invalid phone number
    return;
  }

  if (credentials.email && !emailPattern.test(credentials.email)) {
    toast.error("Invalid email address");  // Display error for invalid email
    return;
  }

  // If email is not provided, generate one
  if (!credentials.email) {
    const generatedEmail = `${credentials.phone}@unidentified.com`;
    credentials.email = generatedEmail;
  ...
  try {
    ...
    const phoneResponse = await axios.get("http://localhost:9000/store/getEmailforPassword", {
      params: {
        phoneNo: credentials.phone
      }
    });

    // Print the phone response to the console
    // console.log("Phone Response: ", phoneResponse.data);

    const response = await medusaClient.auth.exists(credentials.email);
    if (phoneResponse.data.customer || response.exists) {
      toast.error("Customer already exists. Please log in.");
      setCurrentView(LOGIN_VIEW.SIGN_IN);
      router.push("/account");
      return;
  ...
  return (
    ...
    <div className="max-w-sm flex flex-col items-center mt-12">
      ...
      {/* {!showOtpInput && ( */}
        <div>
          {/* <h1 className="text-large-semi uppercase text-center mb-6">Become an Anikaa Member</h1> */}
          <p className="text-center text-base-regular text-gray-700 mb-4">
            Create your Anikaa Member profile, and get access to an enhanced shopping experience.
          </p>
...
/>
  ...
  {...register("phone", {
    pattern: {
      ...
      message: "Please enter a valid phone number in format +91XXXXXXXXXX",
    },
  })}
  autoComplete="tel"
  errors={errors}
...
/>
            ...
            <span className="text-center text-gray-700 text-xsmall-regular mt-6">
              By creating an account, you agree to Anikaa&apos;s{" "}
