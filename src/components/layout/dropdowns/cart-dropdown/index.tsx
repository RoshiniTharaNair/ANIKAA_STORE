...
const CartDropdown = () => {
  ...
  const transformThumbnailUrl = (url: string | null): string => {
    ...
    return url.replace("http://localhost:9000/uploads", "https://anikaa.com/backend/uploads");
  };

  return (
    <div className="h-full z-50" onMouseEnter={open} onMouseLeave={close}>
      <Popover className="relative h-full">
      <Popover.Button className="h-full mt-1">
          <div className="relative flex items-center">
            <Link className="menu-icon" href="/explore/cart" style={{ textDecoration: 'none' }}>
              {/* Displaying FontAwesome shopping cart icon */}
              <FontAwesomeIcon icon={faShoppingCart} className="shopping-cart-icon" />
              {totalItems > 0 && (
                <span className="cart-badge">{totalItems}</span> // Badge to display totalItems
              )}
            </Link>
          </div>
        </Popover.Button>
            ...
            {cart && items?.length ? (
              <>
                ...
                <div className="p-4 flex flex-col gap-y-4 text-small-regular">
                  ...
                  <Link href="/explore/cart" passHref>
