...
const Overview = ({ customer, orders }: OverviewProps) => {
  ...
  const signedInAs = customer?.email.endsWith("@unidentified.com")
        ...
        <div className="text-xl-semi flex justify-between items-center mb-4">
          <span data-testid="welcome-message" data-value={customer?.first_name}>Hello {customer?.first_name}</span>
           <span className="text-small-regular text-ui-fg-base">
            Signed in as:{" "}
            <span className="font-semibold" data-testid="customer-email" data-value={signedInAs}>
              {signedInAs}
            </span>
          </span>
        </div>
                  ...
                  orders.slice(0, 5).map((order) => {
                    return (
                      <li key={order.id} data-testid="order-wrapper" data-value={order.id}>
                        <LocalizedClientLink
                          href={`/explore/account/orders/details/${order.id}`}
                        >
