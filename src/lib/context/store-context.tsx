...
export const StoreProvider = ({ children }: StoreProps) => {
  ...
  const getSearchParams = () => {
    if (typeof window !== 'undefined') {
      ...
      return {
        cart_id: searchParams.get("cart_id"),
  ...
  const { cart_id: onboardingCartId, onboarding: isOnboarding } = getSearchParams();
    ...
    addLineItem.mutate(
      ...
      },
      {
        onSuccess: async ({ cart }) => {
          setCart(cart);
          storeCart(cart.id);
          console.log("Add Line Item Response: ", cart); // Log the response to the console
  
          // Find and log the ID of the item with the matching variant_id
          const matchingItem = cart.items.find((item) => item.variant_id === variantId);
          if (matchingItem) {
            console.log("Matching Line Item ID: ", matchingItem.id);
            console.log("Design Preferences: ", design_preferences);
            console.log("Design Preferences: ", design_images);

            // Prepare the request body for the update API
            const requestBody = {
              id: matchingItem.id,
              design_preference: design_preferences,
              design_images: design_images
            };
